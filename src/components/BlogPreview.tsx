...
import { BlogGenerationSettings, BLOG_DIMENSIONS, STYLE_PREVIEWS } from '../types/blog';
...
function DraggableElement({ id, children, initialPosition, onPositionChange }: DraggableElementProps) {
  ...
  const [position, setPosition] = React.useState(initialPosition || { x: 50, y: 50 });
...
export function BlogPreview({ settings, onTextMove, onLogoMove, previewImage }: BlogPreviewProps) {
  const containerRef = React.useRef<HTMLDivElement>(null);
  const [dimensions, setDimensions] = React.useState({
    width: BLOG_DIMENSIONS.width,
    height: BLOG_DIMENSIONS.height,
    aspectRatio: BLOG_DIMENSIONS.aspectRatio
  });
  ...
  React.useEffect(() => {
    const updateDimensions = () => {
      ...
      let { width, height, aspectRatio } = BLOG_DIMENSIONS;
